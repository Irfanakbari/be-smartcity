...
const admGetPasien = async (req, res) => {
    ...
    try {
        koneksi.query(
            "SELECT * FROM edokter_reservasi JOIN users ON edokter_reservasi.nik_pasien=users.nik JOIN edokter_dokter ON edokter_reservasi.id_dokter = edokter_dokter.id_dokter WHERE edokter_reservasi.id_rs = ? ORDER BY tanggal DESC",
            [me[0].id_rs],
            (err, results) => {
                if (err) {
                    res.status(500).send(err);
